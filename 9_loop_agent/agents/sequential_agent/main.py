...
# --- FastAPI Application ---
api = FastAPI(
    title="Sequential Agent Service",
    description="API for running the Sequential Agent."
)
...
async def execute_agent_run(query: str):
    ...
    try:
        ...
        return responses[-2] if len(responses) > 1 else responses[-1]
...
@api.post("/run-sequantial-agent", response_model=Dict[str, Any])
async def run_sequantial_agent(query: str) -> Dict[str, Any]:
    """
    Triggers the sequantial agent with the provided user input.
