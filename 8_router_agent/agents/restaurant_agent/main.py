...
# --- FastAPI Application ---
api = FastAPI(
    title="Restaurant Agent Service",
    description="API for running the Restaurant Agent."
)
...
async def setup_session_and_runner(user_id: str):
    ...
    session = await service.create_session(
        app_name=restaurant_agent.name, 
        user_id=user_id
    )
    ...
    logger.info(f"Setup session for agent: '{restaurant_agent.name}', user: '{user_id}', session: '{session.id}'...")
...
@api.post("/run-restaurant-agent", response_model=Dict[str, Any])
async def run_restaurant_agent(query: str) -> Dict[str, Any]:
    """
    Triggers the restaurant agent with the provided user input.
