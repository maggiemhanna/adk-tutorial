...
# --- FastAPI Application ---
api = FastAPI(
    title="Hotel Agent Service",
    description="API for running the Hotel Agent."
)
...
async def setup_session_and_runner(user_id: str):
    ...
    logger.info(f"Setup session for agent: '{hotel_agent.name}', user: '{user_id}', session: '{session.id}'...")
...
@api.post("/run-hotel-agent", response_model=Dict[str, Any])
async def run_hotel_agent(query: str) -> Dict[str, Any]:
    """
    Triggers the hotel agent with the provided user input.
