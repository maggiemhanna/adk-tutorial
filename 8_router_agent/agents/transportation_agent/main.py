...
APP_NAME = "transportation_agent"

# --- FastAPI Application ---
api = FastAPI(
    title="Transportation Agent Service",
    description="API for running the Transportation Agent."
)
...
async def setup_session_and_runner(user_id: str):
    ...
    logger.info(f"Setup session for agent: '{transportation_agent.name}', user: '{user_id}', session: '{session.id}'...")
...
@api.post("/run-transportation-agent", response_model=Dict[str, Any])
async def run_transportation_agent(query: str) -> Dict[str, Any]:
    """
    Triggers the transportation agent with the provided user input.
