...
api = FastAPI(
    ...
    description="API for running the Weather Agent."
)
...
async def setup_session_and_runner(user_id: str):
    ...
    logger.info(f"Setup session for agent: '{weather_agent.name}', user: '{user_id}', session: '{session.id}'...")
...
@api.post("/run-weather-agent", response_model=Dict[str, Any])
async def run_weather_agent(query: str) -> Dict[str, Any]:
    """
    Triggers the weather agent with the provided user input.
